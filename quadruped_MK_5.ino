...
const float rollkp = 0.2;
const float rollki = 0;
const float rollkd = 0.001;


const float pitchkp = 0.2;
const float pitchki = 0;
const float pitchkd = 0.001;
...
void loop() {
  ...
  driver1.setPWM(2,0,327);
  driver1.setPWM(4,0,327);
  driver1.setPWM(5,0,227);
  driver1.setPWM(6,0,327);

  driver2.setPWM(0,0,353);
  driver2.setPWM(1,0,307);
  driver2.setPWM(2,0,347);
  driver2.setPWM(4,0,327);
  driver2.setPWM(5,0,317);
  driver2.setPWM(6,0,347);
  */
  /*
...
void actuate(int a, float x, float y, float z)
{
  if (a == 1)
  {
    ...
    driver1.setPWM(4, 0, int(mapping(asin(Lc / sqrt(y * y + z * z)) + atan(z / y), -pi / 2, pi / 2, SERVOMAX, SERVOMIN) / 1000000 * FREQUENCY * 4096 )+20);
    driver1.setPWM(5, 0, int(mapping(acos((Lf * Lf + x1 * x1 + y1 * y1 - Lt * Lt) / (2 * Lf * sqrt(x1 * x1 + y1 * y1))) - atan(x1 / y1), -pi / 2, pi / 2, SERVOMAX, SERVOMIN) / 1000000 * FREQUENCY * 4096) - 100);
    driver1.setPWM(6, 0, int(mapping(asin((x * x + y * y + z * z - Lc * Lc - Lt * Lt - Lf * Lf) / (2 * Lf * Lt)), -pi / 2, pi / 2, SERVOMAX, SERVOMIN) / 1000000 * FREQUENCY * 4096)+20);
  }
  if (a == 0)
  {
    lfd.x=x;
    ...
    float x1 = -x;
    float y1 = -sqrt(y * y + z * z - Lc * Lc);
    driver1.setPWM(0, 0, int(mapping(asin(Lc / sqrt(y * y + z * z)) - atan(z / y), -pi / 2, pi / 2, SERVOMIN, SERVOMAX) / 1000000 * FREQUENCY * 4096));
    driver1.setPWM(1, 0, int(mapping(acos((Lf * Lf + x1 * x1 + y1 * y1 - Lt * Lt) / (2 * Lf * sqrt(x1 * x1 + y1 * y1))) - atan(x1 / y1), -pi / 2, pi / 2, SERVOMIN, SERVOMAX) / 1000000 * FREQUENCY * 4096)+20);
    driver1.setPWM(2, 0, int(mapping(asin((x * x + y * y + z * z - Lc * Lc - Lt * Lt - Lf * Lf) / (2 * Lf * Lt)), -pi / 2, pi / 2, SERVOMIN, SERVOMAX) / 1000000 * FREQUENCY * 4096)+20);
    
    Serial.print("LFC: ");
    Serial.print((asin(Lc / sqrt(y * y + z * z)) - atan(z / y))*180/pi);
    Serial.print(" LFF: ");
    Serial.print((acos((Lf * Lf + x1 * x1 + y1 * y1 - Lt * Lt) / (2 * Lf * sqrt(x1 * x1 + y1 * y1))) - atan(x1 / y1))*180/pi);
    Serial.print(" LFT: ");
    Serial.println(asin((x * x + y * y + z * z - Lc * Lc - Lt * Lt - Lf * Lf) / (2 * Lf * Lt))*180/pi);
  ...
  if (a == 3)
  {
    ...
    float x1 = -x;
    float y1 = -sqrt(y * y + z * z - Lc * Lc);
    driver2.setPWM(4, 0, int(mapping(asin(Lc / sqrt(y * y + z * z)) + atan(z / y), -pi / 2, pi / 2, SERVOMIN, SERVOMAX) / 1000000 * FREQUENCY * 4096) + 30);
    driver2.setPWM(5, 0, int(mapping(acos((Lf * Lf + x1 * x1 + y1 * y1 - Lt * Lt) / (2 * Lf * sqrt(x1 * x1 + y1 * y1))) - atan(x1 / y1), -pi / 2, pi / 2, SERVOMAX, SERVOMIN) / 1000000 * FREQUENCY * 4096) -10);
    driver2.setPWM(6, 0, int(mapping(asin((x * x + y * y + z * z - Lc * Lc - Lt * Lt - Lf * Lf) / (2 * Lf * Lt)), -pi / 2, pi / 2, SERVOMAX, SERVOMIN) / 1000000 * FREQUENCY * 4096) + 40);
  }
  if (a == 2)
  {
    lbd.x=x;
    ...
    float x1 = -x;
    float y1 = -sqrt(y * y + z * z - Lc * Lc);
    driver2.setPWM(0, 0, int(mapping(asin(Lc / sqrt(y * y + z * z)) - atan(z / y), -pi / 2, pi / 2, SERVOMAX, SERVOMIN) / 1000000 * FREQUENCY * 4096) + 60);
    driver2.setPWM(1, 0, int(mapping(acos((Lf * Lf + x1 * x1 + y1 * y1 - Lt * Lt) / (2 * Lf * sqrt(x1 * x1 + y1 * y1))) - atan(x1 / y1), -pi / 2, pi / 2, SERVOMIN, SERVOMAX) / 1000000 * FREQUENCY * 4096)+30);
    driver2.setPWM(2, 0, int(mapping(asin((x * x + y * y + z * z - Lc * Lc - Lt * Lt - Lf * Lf) / (2 * Lf * Lt)), -pi / 2, pi / 2, SERVOMIN, SERVOMAX) / 1000000 * FREQUENCY * 4096) + 40);
  }
}
...
struct point generateBezierCurve(float x0, float y0, float z0, float x2, float y2, float z2, float height, float t)
{
  //0<=t<=1
  struct point p0 = {x0, y0, z0};
  struct point p2 = {x2, y2, z2};
  ...
  curve.x = q0.x * t + q1.x * (1 - t);
  curve.y = q0.y * t + q1.y * (1 - t);
  curve.z = q0.z * t + q1.z * (1 - t);
...
void continuous_creep()
{
  ...
  {
    actuate(1,-s*t/40,-H+60-3*abs(t-20),Lc+20);
    delay(500);
  }
  for(int t=0;t<=40;t+=5)
  {
    actuate(3,s*t/40,-H+60-3*abs(t-20),Lc+20);
    delay(500);
  }
  ...
  while(true)
  {
    ...
    if(thetax>0)   //leftcurve
    {
      ...
      for(int t=0;t<=40;t+=5)
      {
        actuate(1,oldrfd.x+(rfdp.x-oldrfd.x)*t/40,-H+60-3*abs(t-20),oldrfd.z+(rfdp.z+20-oldrfd.z)*t/40);
        delay(500);
      }
      ...
      for(int t=0;t<=40;t+=5)
      {
        actuate(2,oldlbd.x+(lbdp.x-oldlbd.x)*t/40,-H+60-3*abs(t-20),oldlbd.z+(lbdp.z-20-oldlbd.z)*t/40);
        delay(500);
      }
      ...
      for(int t=0;t<=40;t+=5)
      {
        actuate(0,oldlfd.x+(lfdp.x-oldlfd.x)*t/40,-H+60-3*abs(t-20),oldlfd.z+(lfdp.z-20-oldlfd.z)*t/40);
        delay(500);
      }
      ...
      for(int t=0;t<=40;t+=5)
      {
        actuate(3,oldrbd.x+(rbdp.x-oldrbd.x)*t/40,-H+60-3*abs(t-20),oldrbd.z+(rbdp.z+20-oldrbd.z)*t/40);
        delay(500);
      }
    ...
    if(thetax==0)
    {
      for(int t=0;t<=40;t+=5)
      {
        actuate(1,-s+2*s*t/40,-H+60-3*abs(t-20),Lc+20);
        delay(500);
      }
      for(int t=0;t<=40;t+=5)
      {
      ...
      for(int t=0;t<=20;t+=5)
      {
        actuate(0,-s,-H,-Lc+20-2*t);
        actuate(1,0,-H,Lc+20-2*t);
        actuate(2,-s,-H,-Lc+20-2*t);
        actuate(3,0,-H,Lc+20-2*t);
        delay(500);
      }

      for(int t=0;t<=40;t+=5)
      {
        actuate(2,-s+2*s*t/40,-H+60-3*abs(t-20),-Lc-20);
        delay(500);
      }
      for(int t=0;t<=40;t+=5)
      {
        actuate(0,-s+2*s*t/40,-H+60-3*abs(t-20),-Lc-20);
        delay(500);
      }
      ...
      for(int t=0;t<=20;t+=5)
      {
        actuate(0,0,-H,-Lc-20+2*t);
        actuate(1,-s,-H,Lc-20+2*t);
        actuate(2,0,-H,-Lc-20+2*t);
        actuate(3,-s,-H,Lc-20+2*t);
        delay(500);
      }

      for(int t=0;t<=40;t+=5)
      {
        actuate(3,-s+2*s*t/40,-H+60-3*abs(t-20),Lc+20);
        delay(500);
      }
    }
  }
...
void balance()
{
  while (true)
  {
    actuatePID(0, L / 2, -H, -W / 2 - Lc);
    actuatePID(1, L / 2, -H, W / 2 + Lc);
    actuatePID(2, -L / 2, -H, -W / 2 - Lc);
    actuatePID(3, -L / 2, -H, W / 2 + Lc);
    delay(10);
  }
}
...
void trot(int steps)
{
  ...
  for (int i = 1; i <= k; i++)
  {
    for (int temp = 120; temp >= 60; temp -= 5)
    {
      ...
      actuate(0, 2 * s * cos(theta), - H + 60 - 2 * fabs(t - 30), -Lc);
      actuate(3, 2 * s * cos(theta), - H + 60 - 2 * fabs(t - 30), Lc);
      delay(7 );
    }
    for (int temp = 120; temp >= 60; temp -= 5)
    {
      ...
      actuate(1, 2 * s * cos(theta), - H + 60 - 2 * fabs(t - 30), Lc);
      actuate(2, 2 * s * cos(theta), - H + 60 - 2 * fabs(t - 30), -Lc);
      delay(7);
    }
  }
  for (int temp = 60; temp >= 30; temp -= 5)
  {
    ...
    actuate(1, s - s * t / 30, -H, Lc);
    actuate(2, s - s * t / 30, -H, -Lc);
    actuate(0, -s / 2 + s * cos(2 * theta), -H + 30 - 2 * fabs(t - 15), -Lc);
    actuate(3, -s / 2 + s * cos(2 * theta), -H + 30 - 2 * fabs(t - 15), Lc);
    delay(7);
  }
}
*/
...
void trot(int steps)
{
  ...
  for (int temp = 60; temp >= 30; temp -= 5)
  {
    int t = 60 - temp;
    float theta = temp * pi / 180;
    actuate(1, s*t/30, -H + 30 - 2 * fabs(t - 15),(W+2*Lc)/L*s*t/30);
    actuate(2, s*t/30, -H + 30 - 2 * fabs(t - 15),(W+2*Lc)/L*s*t/30);
    actuate(0, -s*t/30, -H, (W+2*Lc)/L*s*t/30);
    actuate(3, -s*t/30, -H, (W+2*Lc)/L*s*t/30);
    delay(10);
  }
  for (int i = 1; i <= k; i++)
  {
    for (int temp = 120; temp >= 60; temp -= 5)
    {
      int t = 120 - temp;
      float theta = temp * pi / 180;     
      actuate(1, s-s*t/30, -H, (W+2*Lc)/L*s-(W+2*Lc)/L*s*t/30);
      actuate(2, s-s*t/30, -H, (W+2*Lc)/L*s-(W+2*Lc)/L*s*t/30);
      actuate(0, -s+s*t/30, - H + 60 - 2 * fabs(t - 30), (W+2*Lc)/L*s-(W+2*Lc)/L*s*t/30);
      actuate(3, -s+s*t/30, - H + 60 - 2 * fabs(t - 30), (W+2*Lc)/L*s-(W+2*Lc)/L*s*t/30);
      delay(10);
    }
    ...
    for (int temp = 120; temp >= 60; temp -= 5)
    {
      int t = 120 - temp;
      float theta = temp * pi / 180;
      actuate(0, s-s*t/30, -H, -(W+2*Lc)/L*s+(W+2*Lc)/L*s*t/30);
      actuate(3, s-s*t/30, -H, -(W+2*Lc)/L*s+(W+2*Lc)/L*s*t/30);
      actuate(1, -s+s*t/30, - H + 60 - 2 * fabs(t - 30), -(W+2*Lc)/L*s+(W+2*Lc)/L*s*t/30);
      actuate(2, -s+s*t/30, - H + 60 - 2 * fabs(t - 30), -(W+2*Lc)/L*s+(W+2*Lc)/L*s*t/30);
      delay(10);
    }
  }
  ...
  for (int temp = 60; temp >= 30; temp -= 5)
  {
    ...
    actuate(1, s-s*t/30, -H, (W+2*Lc)/L*s-(W+2*Lc)/L*s*t/30);
    actuate(2, s-s*t/30, -H, (W+2*Lc)/L*s-(W+2*Lc)/L*s*t/30);
    actuate(0, -s+s*t/30, -H + 30 - 2 * fabs(t - 15), (W+2*Lc)/L*s-(W+2*Lc)/L*s*t/30);
    actuate(3, -s+s*t/30, -H + 30 - 2 * fabs(t - 15), (W+2*Lc)/L*s-(W+2*Lc)/L*s*t/30);
    delay(10);
  }
}
...
void backtrot(int steps)
{
  ...
  for (int temp = 30; temp <= 60; temp += 5)
  {
    float theta = temp * pi / 180;
    int t = 60 - temp;
    actuate(1, - s / 2 + s * cos(2 * theta), -H - sqrt(3)*s / 2 + s * sin(2 * theta), Lc);
    actuate(2, - s / 2 + s * cos(2 * theta), -H - sqrt(3)*s / 2 + s * sin(2 * theta), -Lc);
    actuate(0, s - s * t / 30, -H, -Lc);
    actuate(3, s - s * t / 30, -H, Lc);
    delay(10);
  }
  for (int i = 1; i <= k; i++)
  {
    for (int temp = 60; temp <= 120; temp += 5)
    {
      ...
      actuate(1, s - s * t / 30, -H, Lc);
      actuate(2, s - s * t / 30, -H, -Lc);
      actuate(0, 2 * s * cos(theta), 2 * s * sin(theta) - H - sqrt(3)*s, -Lc);
      actuate(3, 2 * s * cos(theta), 2 * s * sin(theta) - H - sqrt(3)*s, Lc);
      delay(10);
    }
    for (int temp = 60; temp <= 120; temp += 5)
    {
      ...
      actuate(0, s - s * t / 30, -H, -Lc);
      actuate(3, s - s * t / 30, -H, Lc);
      actuate(1, 2 * s * cos(theta), 2 * s * sin(theta) - H - sqrt(3)*s, Lc);
      actuate(2, 2 * s * cos(theta), 2 * s * sin(theta) - H - sqrt(3)*s, -Lc);
      delay(10);
    }
  }
  for (int temp = 30; temp <= 60; temp += 5)
  {
    ...
    actuate(0, s / 2 + s * cos(2 * theta), -H - sqrt(3)*s / 2 + s * sin(2 * theta), -Lc);
    actuate(3, s / 2 + s * cos(2 * theta), -H - sqrt(3)*s / 2 + s * sin(2 * theta), Lc);
    delay(10);
  }
}
...
void spin(float theta)
{
  ...
  if (theta >= -pi / 2 && theta <= pi / 2)
  {
    ...
    for (int t = 0; t <= 60; t+=5)
    {
      ...
      actuate(0, lfd.x, lfd.y + 30 - fabs(t - 30), lfd.z);
      actuate(1, rfd.x, rfd.y, rfd.z);
      actuate(2, lbd.x, lbd.y, lbd.z);
      actuate(3, rbd.x, rbd.y + 30 - fabs(t - 30), rbd.z);
      delay(7);
    }
    //step 2
    for (int t = -60; t <= 0; t+=5)
    {
      ...
      rbd = vec_sub(rbf, rbs0);
      actuate(0, lfd.x, lfd.y, lfd.z);
      actuate(1, rfd.x, rfd.y + 30 - fabs(-t - 30), rfd.z);
      actuate(2, lbd.x, lbd.y + 30 - fabs(-t - 30), lbd.z);
      actuate(3, rbd.x, rbd.y, rbd.z);
      delay(7);
    }
  }
...
void backspin(float theta)
{
  ...
  if (theta <= pi / 2 && theta >= -pi / 2)
  {
    ...
    for (int t = 0; t <= 60; t+=5)
    {
      ...
      rff = quat_rot(y_axis, rff0, theta / 2 * t / 60);
      rbf = quat_rot(y_axis, rbf0, -theta / 2 * t / 60);
      actuate(0, lfd.x, lfd.y + 10 - fabs(t / 3 - 10), lfd.z);
      actuate(1, rfd.x, rfd.y, rfd.z);
      actuate(2, lbd.x, lbd.y, lbd.z);
      actuate(3, rbd.x, rbd.y + 10 - fabs(t / 3 - 10), rbd.z);
      delay(10);
    }
    //step2
    for (int t = -60; t <= 0; t+=5)
    {
      ...
      rbf = quat_rot(y_axis, rbf0, theta / 2 * t / 60);
      actuate(0, lfd.x, lfd.y, lfd.z);
      actuate(1, rfd.x, rfd.y + 10 - fabs(-t / 3 - 10), rfd.z);
      actuate(2, lbd.x, lbd.y + 10 - fabs(-t / 3 - 10), lbd.z);
      actuate(3, rbd.x, rbd.y, rbd.z);
      delay(10);
    }
  }
...
void translateright(int steps)
{
  ...
  for (int temp = 60; temp >= 30; temp -= 5)
  {
    float theta = temp * pi / 180;
    int t = 60 - temp;
    actuate(1, 0, -H + 30 - 2 * fabs(t - 15), s / 2 + s * cos(2 * theta) + Lc - 20);
    actuate(2, 0, -H + 30 - 2 * fabs(t - 15), s / 2 + s * cos(2 * theta) - Lc + 20);
    actuate(0, 0, -H, -s * t / 30 - Lc + 20);
    actuate(3, 0, -H, -s * t / 30 + Lc - 20);
    delay(10);
  }
  for (int i = 1; i <= k; i++)
  {
    for (int temp = 120; temp >= 60; temp -= 5)
    {
      float theta = temp * pi / 180;
      int t = 120 - temp;
      actuate(1, 0, -H, s - s * t / 30 + Lc - 20);
      actuate(2, 0, -H, s - s * t / 30 - Lc + 20);
      actuate(0, 0, - H + 60 - 2 * fabs(t - 30), 2 * s * cos(theta) - Lc + 20);
      actuate(3, 0, - H + 60 - 2 * fabs(t - 30), 2 * s * cos(theta) + Lc - 20);
      delay(10);
    }
    for (int temp = 120; temp >= 60; temp -= 5)
    {
      float theta = temp * pi / 180;
      int t = 120 - temp;
      actuate(0, 0 , -H, s - s * t / 30 - Lc + 20);
      actuate(3, 0, -H, s - s * t / 30 + Lc - 20);
      actuate(1, 0, - H + 60 - 2 * fabs(t - 30), 2 * s * cos(theta) + Lc - 20);
      actuate(2, 0, - H + 60 - 2 * fabs(t - 30), 2 * s * cos(theta) - Lc + 20);
      delay(10);
    }
  }
  for (int temp = 60; temp >= 30; temp -= 5)
  {
    ...
    actuate(1, 0, -H, s - s * t / 30 + Lc - 20);
    actuate(2, 0, -H, s - s * t / 30 - Lc + 20);
    actuate(0, 0, -H + 30 - 2 * fabs(t - 15), -s / 2 + s * cos(2 * theta) - Lc + 20);
    actuate(3, 0, -H + 30 - 2 * fabs(t - 15), -s / 2 + s * cos(2 * theta) + Lc - 20);
    delay(10);
  }
}
...
void translateleft(int steps)
{
  ...
  for (int temp = 30; temp <= 60; temp += 5)
  {
    float theta = temp * pi / 180;
    int t = 60 - temp;
    actuate(1, 0, -H, - s * t / 30 + Lc - 20);
    actuate(2, 0, -H, - s * t / 30 - Lc + 20);
    actuate(0, 0, -H + 30 - 2 * fabs(t - 15), s / 2 + s * cos(2 * theta) - Lc + 20);
    actuate(3, 0, -H + 30 - 2 * fabs(t - 15), s / 2 + s * cos(2 * theta) + Lc - 20);
    delay(10);
  }
...
void walk(int steps)
{
  ...
  for (int t = 0; t <= 20; t += 5)
  {
    actuate(0, sw*t/20, -H+40-4*abs(t-10), -Lc-40);
    actuate(1, -sw / 2 * t / 20, -H, Lc - 40);
    actuate(2, -sw / 2 * t / 20, -H, -Lc - 40);
    actuate(3, -sw / 2 * t / 20, -H, Lc - 40);
    delay(10);
  }
  for (int t = 0; t <= 80; t += 5)
  {
    actuate(0, sw, -H, -Lc - 40 + t);
    actuate(1, -sw / 2, -H, Lc - 40 + t);
    actuate(2, -sw / 2, -H, -Lc - 40 + t);
    actuate(3, -sw / 2, -H, Lc - 40 + t);
    delay(10);
  }
  for (int t = 0; t <= 20; t += 5)
  {
    actuate(0, sw - sw / 2 * t / 20, -H, -Lc + 40);
    actuate(1, -sw / 2 - sw / 2 * t / 20, -H, Lc + 40);
    actuate(2, -sw / 2 - sw / 2 * t / 20, -H, -Lc + 40);
    actuate(3, -sw/2+3*sw/2*t/20, -H+40-4*abs(t-10), Lc+40);
    delay(10);
  }
  for (int t = 0; t <= 20; t += 5)
  {
    actuate(0, sw / 2 - sw / 2 * t / 20, -H, -Lc + 40);
    actuate(1, -sw+2*sw*t/20, -H+40-4*abs(t-10), Lc+40);
    actuate(2, -sw - sw / 2 * t / 20, -H, -Lc + 40);
    actuate(3, sw - sw / 2 * t / 20, -H, Lc + 40);
    delay(10);
  }
  for (int t = 0; t <= 80; t += 5)
  {
    actuate(0, 0, -H, -Lc + 40 - t);
    actuate(1, sw, -H, Lc + 40 - t);
    actuate(2, -3 * sw / 2, -H, -Lc + 40 - t);
    actuate(3, sw / 2, -H, Lc + 40 - t);
    delay(10);
  }
  for (int t = 0; t <= 20; t += 5)
  {
    actuate(0, -sw / 2 * t / 20, -H, -Lc - 40);
    actuate(1, sw - sw / 2 * t / 20, -H, Lc - 40);
    actuate(2, -3*sw/2+5*sw/2*t/20, -H+40-4*abs(t-10), -Lc-40);
    actuate(3, sw / 2 - sw / 2 * t / 20, -H, Lc - 40);
    delay(10);
  }
  for (int i = 1; i <= k; i++)
  {
    for (int t = 0; t <= 20; t += 5)
    {
      actuate(0, -sw/2+3*sw/2*t/20, -H+40+4*abs(t-10), -Lc-40);
      actuate(1, sw / 2 - sw / 2 * t / 20, -H, Lc - 40);
      actuate(2, sw - sw / 2 * t / 20, -H, -Lc - 40);
      actuate(3, -sw / 2 * t / 20, -H, Lc - 40);
      delay(10);
    ...
    {
      actuate(0, sw / 2 - sw / 2 * t / 20, -H, -Lc + 40);
      actuate(1, -sw/2+3*sw/2*t/20, -H+40-4*abs(t-10), Lc+40);
      actuate(2, -sw / 2 * t / 20, -H, -Lc + 40);
      actuate(3, sw - sw / 2 * t / 20, -H, Lc + 40);
      delay(10);
    }
    ...
    for (int t = 0; t <= 20; t += 5)
    {
      actuate(0, -sw / 2 * t / 20, -H, -Lc - 40);
      actuate(1, sw - sw / 2 * t / 20, -H, Lc - 40);
      actuate(2, -sw/2+3*sw/2*t/20, -H+40-4*abs(t-10), -Lc-40);
      actuate(3, sw / 2 - sw / 2 * t / 20, -H, Lc - 40);
      delay(10);
    }
  }
  for (int t = 0; t <= 20; t += 5)
  {
    actuate(0, -sw/2+sw*t/20, -H+40-4*abs(t-10), Lc-40);
    actuate(1, sw / 2 - sw / 2 * t / 20, -H, Lc - 40);
    actuate(2, sw - sw / 2 * t / 20, -H, -Lc - 40);
    actuate(3, -sw / 2 * t / 20, -H, Lc - 40);
    delay(10);
  ...
  for (int t = 0; t <= 20; t += 5)
  {
    ...
    actuate(1, -sw / 2 * t / 20, -H, Lc + 40);
    actuate(2, sw / 2 - sw / 2 * t / 20, -H, -Lc + 40);
    actuate(3, -sw/2+sw/2*t/20, -H+40-4*abs(t-10),Lc+40);
    delay(10);
  }
  for (int t = 0; t <= 20; t += 5)
  {
    actuate(1, -sw/2+sw/2*t/20, -H+40-4*abs(t-10),Lc+40);
    delay(10);
  }
  for (int t = 0; t <= 40; t += 5)
  {
    actuate(0, 0, -H, -Lc + 40 - t);
    actuate(1, 0, -H, Lc + 40 - t);
    actuate(2, 0, -H, -Lc + 40 - t);
    actuate(3, 0, -H, Lc + 40 - t);
    delay(20);
  }
}
